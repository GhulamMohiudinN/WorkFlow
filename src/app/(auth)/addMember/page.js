...
import authAPI from "../../api/auth"; // Update this path
...
function AddMemberContent() {
  ...
  useEffect(() => {
    ...
    // Just set the email field with a placeholder, actual email will come from token validation if needed
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      if (response.token) {
        ...
        const userType = response.user?.userType || "admin";
        localStorage.setItem("role", userType);
      }
      
      // after invite acceptance, direct user to role-dependent dashboard
